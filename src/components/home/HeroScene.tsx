...
export const HeroScene = () => {
  ...
  useEffect(() => {
    ...
    tl
      ...
      .fromTo(colsRef.current,
        { opacity: 0 },
        { opacity: 1, duration: 1, ease: 'power2.out' }, '-=0.5');
  }, []);

  return (
    /* On mobile: flex-col (text stacked above photos)
       On desktop: relative with absolute photo columns on right */
    <div className="w-full bg-black flex flex-col md:block md:relative md:h-screen md:min-h-[700px] md:overflow-hidden">

      {/* TEXT SECTION */}
      <div className="relative z-20 flex flex-col justify-center w-full md:w-[54%] px-8 sm:px-12 lg:px-20 pt-28 pb-12 md:pt-0 md:pb-0 md:h-full md:justify-center shrink-0">

        {/* Badge */}
        <div ref={badgeRef} style={{ opacity: 0 }} className="mb-6 md:mb-8 flex items-center gap-3">
        ...
        <div className="overflow-hidden leading-none mb-6 md:mb-8">
        ...
        {/* Rule */}
        <div ref={ruleRef} className="h-[1px] bg-white/15 w-full mb-6 md:mb-8 origin-left" style={{ transform: 'scaleX(0)' }}></div>

        {/* CTAs + Tagline */}
        <div ref={ctasRef} style={{ opacity: 0 }}>
          <p className="font-playfair italic text-white/60 text-base md:text-xl mb-2 leading-snug">
            Your cut says everything.
          </p>
          <p className="text-white/30 text-xs font-light tracking-wide mb-6 md:mb-8 max-w-xs">
        ...
        <div className="hidden md:flex absolute bottom-8 left-8 sm:left-12 lg:left-20 items-center gap-4 opacity-25">
      ...
      {/* PHOTO COLUMNS — shown on both mobile (strip below text) and desktop (absolute right) */}
      <div
        ref={colsRef}
        style={{ opacity: 0 }}
        className="
          ...
          h-[55vw] max-h-[320px]
          md:absolute md:right-0 md:top-0 md:w-[48%] md:h-full md:max-h-none
        "
      >
        {/* Edge fades */}
        ...
        <div className="absolute top-0 left-0 w-full h-12 md:h-28 bg-gradient-to-b from-black to-transparent z-10 pointer-events-none"></div>
        <div className="absolute bottom-0 left-0 w-full h-12 md:h-28 bg-gradient-to-t from-black to-transparent z-10 pointer-events-none"></div>
